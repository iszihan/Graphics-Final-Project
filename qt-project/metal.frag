...
vec4 illuminateMetal( in vec3 pos , in vec3 camdir)
{
    ...
    float alpha = acos(dot(normal,h));
    float D = (exp(-pow(tan(alpha),2)/pow(m,2)))/(4*pow(m,2)*pow(cos(alpha),4));
    float F = r0 + (1-r0)*pow((1-dot(normal,-camdir)),5);
    float G = min(min(1.0, (2*dot(h,normal)*dot(h,normal))/dot(v,h)), (2*dot(h,normal)*dot(l,normal))/(dot(v,h)));
    float ks = max(0, D*F*G / (dot(v,normal)));
    vec4 objectColor = ambient + dot(normal,l) * diffuse + ks * specular ;

    vec3 refDir = reflect(camdir,normal);
    vec4 reflectColor = texture(envMap, refDir);
    return mix(objectColor,  reflectColor, F);
}
...
void main()
{
    ...
    else
    {
        ...
        col = illuminateMetal(inters, camdir);
    }
